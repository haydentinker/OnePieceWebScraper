...
def character_scraper():
    ...
    with open("character.csv", mode="a") as character_file:
        character_writer = csv.writer(
            character_file, delimiter=",", quotechar='"', quoting=csv.QUOTE_MINIMAL
        )
        ...
        for i in range(0, len(character_table)):
            characters_page = requests.get(
                character_URL + character_table[i][0].get("href")
            )
            ...
            if len(character_information) > 7:
                if (
                    clean_string(
                        ...
                        .find("div", class_="pi-data-value")
                        .text
                    )
                    != "N/A"
                ):
                    character_map = {
                        "English_name": clean_string(
                            character_information[2]
                            .find("div", class_="pi-data-value")
                            .text
                        ),
                        ...
                        "Affiliations": clean_string(
                            character_information[4]
                            .find("div", class_="pi-data-value")
                            .text
                        ),
                        "Occupations": clean_string(
                            character_information[5]
                            .find("div", class_="pi-data-value")
                            .text
                        ),
